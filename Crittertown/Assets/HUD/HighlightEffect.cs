...
public class HighlightEffect : MonoBehaviour {
	...
	void OnRenderImage(RenderTexture src, RenderTexture dest) {
		if(!lameMode && UI.Instance.highlightedObjects.Count > 0) {
			...
			foreach (KeyValuePair<GameObject, Color> kv in UI.Instance.highlightedObjects) {
				if(kv.Key == null) {
					nullKeys.Add(kv.Key);
				} else {
					foreach(Material m in kv.Key.renderer.materials) {
						m.SetColor("_SolidColor", kv.Value);
					}
				}
			}
